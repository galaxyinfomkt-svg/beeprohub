...
export default function Header() {
  ...
  return (
    <>
      {/* Top Bar */}
      <div className="bg-dark text-white text-xs py-2 hidden sm:block relative z-[1001]">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="bg-primary text-dark font-bold px-3 py-0.5 rounded-full text-[10px] animate-pulse">Teste Gratis por 14 Dias!</span>
          </div>
          <div className="flex items-center gap-4">
            <LanguageSwitcher />
            <a href={LOGIN_URL} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-primary transition-colors">JA TENHO CONTA</a>
          </div>
        </div>
      </div>

      {/* Main Nav */}
      <header className={`sticky top-0 z-[1000] transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-lg shadow-lg border-b border-gray-100" : "bg-white/90 backdrop-blur-md border-b border-gray-200/50"}`}>
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center gap-3">
              <a href={PHONE_LINK} className="text-sm font-bold text-dark">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[999] bg-white flex flex-col pt-20 overflow-y-auto">
          ...
          <div className="p-6 flex flex-col gap-4 mt-auto border-t border-gray-100">
            ...
            <Link href={`/${locale}/contact`} onClick={() => setMobileOpen(false)} className="btn-primary text-center text-lg">
              {t("getDemo")}
            </Link>
            <a href={LOGIN_URL} target="_blank" rel="noopener noreferrer" className="text-center text-gray-500 text-sm">JA TENHO CONTA &rarr;</a>
          </div>
        </div>
      )}
    </>
